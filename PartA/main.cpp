...
{
  ofstream readings_File; 
  string fname = "readings.txt";  
  readings_File.open(fname);
  ...
  readings_File<<"Input matrix of size : "+to_string(N)+"\n";
  // Input matrix A
  ...
  if(N<=10000){
    ...
    string msgRef = "Reference execution time: " + 
      to_string((double)TIME_DIFF(std::chrono::microseconds, begin, end) / 1000.0) + " ms\n"; 
